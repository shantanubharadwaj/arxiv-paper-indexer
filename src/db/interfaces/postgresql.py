...
class PostgreSQLDatabase(BaseDatabase):
    ...
    def startup(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Attempting to connect to PostgreSQL at : {self.config.databse_url.split('@')[1] if '@' in self.config.databse_url else 'localhost'}"
            )
            
            self.engine = create_engine(
                self.config.databse_url,
    ...
    def get_session(self) -> Generator[Session, None, None]:
        ...
        if not self.session_factory:
            raise RuntimeError("Databse not initialized. Call startup() first.")
